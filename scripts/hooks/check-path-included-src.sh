...
run_self_tests() {
    ...
    mkdir -p "$sandbox/repo/a/b" "$sandbox/repo/crates/cli/src" "$sandbox/repo/crates/cli/tests" "$sandbox/etc"
    printf 'pub struct Cli;\n' > "$sandbox/repo/crates/cli/src/cli.rs"
    ...
    violations=()
    resolve_path_attr "$repo_root/crates/cli/tests/e2e.rs" "../src/cli.rs"
    append_path_attr_violation
    resolved="$resolved_path_attr"
    assert_eq "$repo_root/crates/cli/src/cli.rs" "$resolved" "relative path inside repo is allowed"
    ...
    printf '#[path = "../src/../../../../etc/passwd"]\nmod passwd;\n' > "$repo_root/crates/cli/tests/e2e.rs"
    queue_files=()
    queue_tests=()
    seen_entries=""
    violations=()
    enqueue_test_src_includes "$repo_root/crates/cli/tests/e2e.rs"
