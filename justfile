...
_check_writable path attempted:
    ...
    echo >&2 "Adjust filesystem_sandbox.extra_writable in ~/.config/article-scraper/config.toml to include ${resolved_path}."
...
check-version-bumped:
    ...
    current=$(cargo metadata --no-deps --format-version 1 \
        | jq -r '.packages[] | select(.name == "article-scraper") | .version')
...
test-e2e:
    just check-cargo-target-writable
    just check-nextest-state-writable
    cargo nextest run --package article-scraper --test e2e --all-features
...
install:
    ...
    target_dir="${CARGO_TARGET_DIR:-{{_repo_root}}/target}"
    just check-cargo-target-writable
    cargo build --release --all-features -p article-scraper
